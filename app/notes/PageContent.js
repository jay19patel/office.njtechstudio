...
import { useNotesData } from '@/utils/hooks';
...
function NotesContent() {
    ...
    // Pass searchQuery to hook
    const { data: notes = [], isLoading } = useNotesData(searchQuery);
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {notes.map(note => (
                        <Link href={`/notes/${note.id}`} key={note.id} className="group block bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all h-64 flex flex-col relative overflow-hidden">
